...
function RPLSeaUS6() {
  ...
  const normalizeCoordinates = (latitude, longitude, region = 'US') => {
    // For US West Coast coordinates, longitude should be negative
    if (region === 'US' && longitude > 0 && longitude > 100) {
      return [latitude, -longitude];
    }
    return [latitude, longitude];
  };

  // Then modify your useEffect where you process the polyline data:
  useEffect(() => {
    let interval: NodeJS.Timeout;

    const fetchPolylineData = async () => {
      try {
        const response = await fetch(`${apiBaseUrl}${port}/sea-us-rpl-s6`);
        const result = await response.json();
        ...
        if (Array.isArray(result) && result.length > 0) {
          ...
          // Build positions for the polyline with coordinate normalization
  ...
  return (
    <>
      <CableCutMarkers cableSegment="seaus6" />
