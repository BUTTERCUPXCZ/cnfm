...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0, 'Distance out of bounds (LFES on seabed)')
    .max(2.034, 'Distance cannot exceed Songkhla BU (BU2)'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment4SJC: React.FC<Segment4SJCProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/sjc-rpl-s4`, {
