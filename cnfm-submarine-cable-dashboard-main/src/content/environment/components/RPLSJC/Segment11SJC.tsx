...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0, 'Distance out of bounds: Chung Hom Kok BU (BU5)')
    .max(285.57, 'Distance cannot exceed Shantou BU (BU6)'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment11SJC: React.FC<Segment11SJCProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/sjc-rpl-s11`, {
