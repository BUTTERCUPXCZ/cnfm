...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0, 'Distance out of bounds: Nasugbu BU (BU4)')
    .max(363.912, 'Distance cannot exceed Chung Hom Kok BU (BU5)'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment9SJC: React.FC<Segment9SJCProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/sjc-rpl-s9`, {
