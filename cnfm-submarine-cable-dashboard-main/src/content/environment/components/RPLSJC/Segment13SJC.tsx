...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0.263, 'Distance out of bounds (LP: Chikura, Japan)')
    .max(3505.069, 'Distance cannot exceed Shantou BU (BU6)'),
...
const Segment13SJC: React.FC<Segment13SJCProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/sjc-rpl-s13`, {
