...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0.196, 'Distance out of bounds (BMH: Chung Hom Kok, Hong Kong)')
    .max(707.184, 'Distance cannot exceed Chung Hom Kok BU (BU5)'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment10SJC: React.FC<Segment10SJCProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/sjc-rpl-s10`, {
