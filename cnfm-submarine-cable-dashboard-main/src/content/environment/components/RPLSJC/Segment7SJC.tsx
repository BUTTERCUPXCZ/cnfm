...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0, 'Distance out of bounds: Telisai BU (BU3)')
    .max(1130.844, 'Distance cannot exceed Nasugbu BU (BU4)'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment7SJC: React.FC<Segment7SJCProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/sjc-rpl-s7`, {
