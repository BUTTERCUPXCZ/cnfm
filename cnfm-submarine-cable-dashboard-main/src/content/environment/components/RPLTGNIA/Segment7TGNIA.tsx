...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0, 'Distance out of bounds (Clump Weight)')
    .max(2.267, 'Distance cannot exceed Branching Unit'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment7TGNIA: React.FC<Segment7TGNIAProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/tgnia-rpl-s7`, {
