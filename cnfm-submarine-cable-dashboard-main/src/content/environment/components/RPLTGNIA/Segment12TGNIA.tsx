...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(2.578, 'Distance out of bounds (A/C)')
    .max(13.92, 'Distance cannot exceed Transition'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment12TGNIA: React.FC<Segment12TGNIAProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/tgnia-rpl-s12`, {
