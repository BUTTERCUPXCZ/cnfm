...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0, 'Distance out of bounds (BU5)')
    .max(1196.039, 'Distance cannot exceed BU6.A'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment6TGNIA: React.FC<Segment6TGNIAProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/tgnia-rpl-s6`, {
