...
const validationSchema = Yup.object({
  kmValue: Yup.number()
    ...
    .min(0, 'Distance out of bounds (Tanah Merah, Singapore)')
    .max(301.273, 'Distance cannot exceed BU1'),
  cutType: Yup.string().required('Cut type selection is required')
});

const Segment1TGNIA: React.FC<Segment1TGNIAProps> = ({
  ...
  const fetchCableData = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}${port}/tgnia-rpl-s1`, {
